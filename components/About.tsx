  ...
  return (
    <section id="about" className="py-20" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`relative rounded-[2.5rem] overflow-hidden group transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="flex-1">
              <h2 className={`text-3xl md:text-5xl font-bold mb-8 text-white tracking-tight ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>About Me</h2>
              <div className={`space-y-6 text-gray-300 leading-relaxed text-lg md:text-xl font-light opacity-90 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
            ...
            <div className={`w-full md:w-1/3 grid grid-cols-1 gap-4 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '0.4s' }}>
                {items.map((item, idx) => (
                    <div key={idx} className="group/item bg-black/20 backdrop-blur-xl p-5 rounded-2xl border border-white/5 flex items-center gap-5 hover:bg-white/10 transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1 hover:border-white/10 cursor-default">
